...
def mysql_initailize():
    user = 'acha'
    passwd = 'acha09!!'
    conn = pymysql.connect(host='acha.io', port=3306, user=user, password=passwd, db='acha', charset='utf8')
    global cur
    cur = conn.cursor()
...
def get_store_info_mysql(store_id, *args):
    ...
    cur.execute('SELECT %s FROM StoreLeftJoinAlarmTalk WHERE UUID = UNHEX(%s)' % (','.join(columns), store_id))
    res = cur.fetchone()
    print(res)
    return res
...
def get_feedback_list(start, minute=10):
    ...
    for reserv in reserv_list:
        ...
        res.append({'token': reserv['reservToken'],
                    ...
                    'send_time': reserv['reservTime'] + timedelta(hours=33)}) # 24(1day) + 9(UTC)
